...
class RecordAuthResponse(Generic[T]):
    def __init__(
        self,
        token: str,
        record: T,
...
class RecordService(CrudService[Record]):
    ...
    def decode(self, data: dict[str, Any]) -> Record:
        return Record(data, auto_snake_case=self.client.auto_snake_case)
    ...
    def subscribe_one(
        ...
        """Subscribe to realtime changes of a single record from the collection."""
        return self.client.realtime.subscribe(
        ...
        if auth_providers_data:
            auth_providers = [
                ...
                for auth_provider in map(
                    apply_pythonic_keys, auth_providers_data
                )
            ]
