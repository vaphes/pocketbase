...
class RecordService(CrudService):
    ...
    def get_file_url(
        ...
        if query_params:
            result += f"?{urlencode(query_params)}"
        return result
    ...
    def auth_with_password(
        ...
        """
        body_params |= {"identity": username_or_email, "password": password}
        response_data = self.client.send(
    ...
    def confirmEmailChange(
        ...
        """
        body_params |= {"token": token, "password": password}
        self.client.send(
